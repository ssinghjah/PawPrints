...
DATE = "2024-07-02" # Year, Month, Date
...
def processNemo(options):
    ...
    if options.gps_log is not None:
        ...
        if options.gps_source == "companion":
            ...
            interp_locs = time_merger.merge_locs(processed_nemo_df["companion_abs_time_ms"].to_list(), gps_data["gps_times"].to_list(), gps_data["latitude"], gps_data["longitude"], gps_data["altitude"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nemo-gps-path', type = str, default = DEFAULT_OUTPUT_PATH, help='Nemo GPS path')
