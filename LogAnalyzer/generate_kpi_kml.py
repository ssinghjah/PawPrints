...
def generate_kml(options):
    ...
    if options.log_type == "per-cell-kpi":
        ...
        log_path = os.path.join(options.workarea, options.log_name)
        log_pd = pd.read_csv(log_path)
        # Filter by PCI
        if options.pci == "connected":
            indices = log_pd["is_connected"] == 1
            kpi_gps_pd = log_pd.loc[indices][["latitude", "longitude", "altitude", "rsrp"]]
            # kpi_log = log_pd.loc[indices][options.kpi].to_list()
            # numEntries = len(kpi_log)
            # kpi_log = log_pd.loc[indices][options.kpi].to_list()
            # gps_log = [[log_pd.iloc[i]["companion_abs_time"], log_pd.iloc[i]["longitude"], log_pd.iloc[i]["latitude"], log_pd.iloc[i]["altitude"]] for i in range(numEntries)]
    ...
    for i in range(numRows):
        ...
        if "line" in DRAW_TYPE:
            if i == numRows - 1:
                ...
                geom.description = f'<p>{options.kpi} = {kpi} {options.kpi_units}</p><p>(lat, lon) = ({lat},{lon})</p><p> altitude = {alt} m</p>'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--log-name', type=str, default = DEFAULT_LOG_NAME, help="Merged log name. Default is " + DEFAULT_LOG_NAME)
